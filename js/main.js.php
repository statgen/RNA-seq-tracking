...
//Default qc_measure to draw histogram
var default_qc_measure = "High Quality Exonic Rate";
var default_field = "high_quality_exonic_rate";
...
$(document).ready(function () {
  ...
  drawStudySummary("Proteomics", "summary_barchart_proteomics", colors[5]);
  ...
  metrics_table = new Tabulator("#qc_metrics_table", {
    ...
    rowFormatter: function(row){
      if(row.getData().full_attribute == default_qc_measure) {
        row.select();
      }
    },
  });
...
<?php } ?>
  ...
  //Draw default histogram using pre-defined qc measure
  ...
  param["field"] = default_field;
  param["study"] = "All studies";
  param["label"]= default_qc_measure;
  drawHistogram(param);
...
function updateSummary() {
  ...
  for(var i=0; i<jsonData.length; i++) {
	...
	break;
      case "Baylor":
        total_baylor+=Number(jsonData[i]["num"]);
	break;
      case "NYGC":
 	total_nygc+=Number(jsonData[i]["num"]);
	break;
      default:
    }
	...
	break;
      case "Proteomics":
        total_proteomics+=Number(jsonData[i]["num"]);
        break;
      default:
    }
    total=total_rna+total_methylation+total_metabolomics+total_proteomics;
    ...
    $("#total-proteomics").text(total_proteomics.toLocaleString('en'));
    ...
    $("#baylor-total").text(total_baylor.toLocaleString('en'));
    $("#nygc-total").text(total_nygc.toLocaleString('en'));
...
function drawProgressOverTime() {
  ...
  var linechart = Highcharts.chart('timeline_chart', options_timeline);
}
...
function studyDropdown(topic="RNA-seq", firstSel="All studies", addCenter=false) {
  ...
  if(addCenter) {
    ...
    selection += '<OPTION value="NYGC">NYGC(center)</OPTION>';
  }
  return selection;
}
